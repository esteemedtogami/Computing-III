...
BigInt operator/(BigInt left, BigInt right)
{
	...
	// If we are dividing by 1...
	if (right.data.at(0) == '1' && right.data.length() == 1)
	{
		return left;
	}

	//Here's where the actual division begins:
	BigInt BILeftover1, BILeftover2;
	...
	for (int i = 0; i < left.data.length(); i++)
	{
		char cDividend;
		int iDividend;
		int iDivisor = stoi(right.data);
		int iPlaced_into_Result;
		char cPlaced_into_Result;
		int under_divisor;
		int iTo_leftover;
		char cTo_leftover;
		...
		if (i == 0)
		{
			cDividend = left.data.at(i);
			iDividend = cDividend - 48;

		}
		else
		{
			BILeftover1.data.push_back(left.data.at(i));
			iDividend = stoi(BILeftover1.data);
		}

		iPlaced_into_Result = iDividend / iDivisor;
		cPlaced_into_Result = iPlaced_into_Result + 48;

		if (i == 0)
		{
			BIResult.data.replace(0, 1, 1, cPlaced_into_Result);
		}
		else
		{
			BIResult.data.push_back(cPlaced_into_Result);
		}

		under_divisor = iPlaced_into_Result * iDivisor;
		iTo_leftover = iDividend - under_divisor;
		cTo_leftover = iTo_leftover + 48;

		//BILeftover2.data.replace(0, 1, 1, cTo_leftover);
		BILeftover1 = iTo_leftover;
	...
	for (;;)
	{
		if (BIResult.data.at(0) == '0')
		{
			BIResult.data.erase(0, 1);
		}
		else
		{
			break;
